function getMelodyArray(melodyIndex, keyboard) {	
	var divIdCoreForKeyboard = {"timeage":"TimeAge","timecountry":"TimeCountry","timepublications":"TimePublications","keypercent":"KeyPercent","countrieskey":"CountriesKey","timekey":"TimeKey","agekey":"AgeKey","ngramcomposer":"NgramComposer"};
	var ngram = $("#Keyboard" + divIdCoreForKeyboard[keyboard] + "Container .gwt-TextBox").val();
...
var renderInput = function(keyboard) {
	...
	if (navigator.appName == 'Microsoft Internet Explorer' || getMelodyArray(1,"timeage") != null) return;
	if (navigator.appName == 'Microsoft Internet Explorer' || getMelodyArray(1,"timecountry") != null) return;
	if (navigator.appName == 'Microsoft Internet Explorer' || getMelodyArray(1,"timepublications") != null) return;
	if (navigator.appName == 'Microsoft Internet Explorer' || getMelodyArray(1,"keypercent") != null) return;
	if (navigator.appName == 'Microsoft Internet Explorer' || getMelodyArray(1,"countrieskey") != null) return;
	if (navigator.appName == 'Microsoft Internet Explorer' || getMelodyArray(1,"timekey") != null) return;
	if (navigator.appName == 'Microsoft Internet Explorer' || getMelodyArray(1,"agekey") != null) return;
	if (navigator.appName == 'Microsoft Internet Explorer' || getMelodyArray(1,"ngramcomposer") != null) return;
...
function switchNgramRepresentation(keyboard) {
	
	var divIdCoreForKeyboard = {"timeage":"TimeAge","timecountry":"TimeCountry","timepublications":"TimePublications","keypercent":"KeyPercent","countrieskey":"CountriesKey","timekey":"TimeKey","agekey":"AgeKey","ngramcomposer":"NgramComposer"};
	var ngramType =  $("#ngram" + divIdCoreForKeyboard[keyboard] + "Container select").val();
	var ngram = $("#Keyboard" + divIdCoreForKeyboard[keyboard] + "Container  .gwt-TextBox").val();
...
function onGwtReady() {
	renderInput("timeage");
	renderInput("timecountry");
	renderInput("timepublications");
	renderInput("keypercent");
	renderInput("countrieskey");
	renderInput("timekey");
	renderInput("agekey");
	renderInput("ngramcomposer");
	flashPianoInit();

	
	
	var a = {"#ngramTimeAgeContainer select","#ngramTimeCountryContainer select","#ngramTimePubliationsContainer select","#ngramKeyPercentContainer select","#ngramCountriesKeyContainer select","#ngramTimeKeyContainer select","#ngramAgeKeyContainer select","#ngramNgramComposerContainer select"};
	var b = {"#KeyboardTimeAgeContainer  .gwt-TextBox","#KeyboardTimeCountryContainer  .gwt-TextBox","#KeyboardTimePublicationsContainer  .gwt-TextBox","#KeyboardKeyPercentContainer  .gwt-TextBox","#KeyboardCountriesKeyContainer  .gwt-TextBox","#KeyboardTimeKeyContainer  .gwt-TextBox","#KeyboardAgeKeyContainer  .gwt-TextBox","#KeyboardNgramComposerContainer  .gwt-TextBox"};
	var c = {"timeage","timecountry","timepublications","keypercent","countrieskey","timekey","agekey","ngramcomposer"};
	...
	contd.live('keyup', function(e){
		...
		if (c == 8 && playedMelody.length > 0) {
			...
			clearTimeout(timer1);
			timer1 = setTimeout("renderInput(c[i]);", 300);
			return;
		}
...
var renderMel = function(mel_str, canvas,keyboard) {
...
var playedNote = function(pitch,keyboard) {
	prevNotePiano = pitch;
	
	var divIdCoreForKeyboard = {"timeage":"TimeAge","timecountry":"TimeCountry","timepublications":"TimePublications","keypercent":"KeyPercent","countrieskey":"CountriesKey","timekey":"TimeKey","agekey":"AgeKey","ngramcomposer":"NgramComposer"};
	var ngramType =  $("#ngram" + divIdCoreForKeyboard[keyboard] + "Container select").val();
	var prevNav = $("#Keyboard" + divIdCoreForKeyboard[keyboard] + "Container .gwt-TextBox").val();
	...
	if (playedMelody.length > 0) {
		...
		$("#Keyboard" + divIdCoreForKeyboard[keyboard] + "Container .gwt-TextBox").val(newNgram);
	} else /* if (prevNgram != "") {
		var newNgram = prevNgram + ", " + pitch;
		$("#contentContainer .gwt-TextBox").val(newNgram);
	} else if (prevNgram == "") */ {
		$("#Keyboard" + divIdCoreForKeyboard[keyboard] + "Container .gwt-TextBox").val(pitch);
	}
	renderInput(keyboard);
...
function onScoreKeyPress(e,keyboard) {
...
function addNoteAfter(noteIndex,keyboard) {
	var ints = getMelodyArray(0,keyboard);
	var divIdCoreForKeyboard = {"timeage":"TimeAge","timecountry":"TimeCountry","timepublications":"TimePublications","keypercent":"KeyPercent","countrieskey":"CountriesKey","timekey":"TimeKey","agekey":"AgeKey","ngramcomposer":"NgramComposer"};
	var ngram = $("#Keyboard" + divIdCoreForKeyboard[keyboard] + "Container 
			 .gwt-TextBox").val();
...
function changeSelectedNote(amount,keyboard) {
	var ints = getMelodyArray(0,keyboard);
...
function changeSelectedNoteDuration(amount,keyboard) {
	var ints = getMelodyArray(0,keyboard);
	var divIdCoreForKeyboard = {"timeage":"TimeAge","timecountry":"TimeCountry","timepublications":"TimePublications","keypercent":"KeyPercent","countrieskey":"CountriesKey","timekey":"TimeKey","agekey":"AgeKey","ngramcomposer":"NgramComposer"};
	var ngramType =  $("#ngram" + divIdCoreForKeyboard[keyboard] + "Container select").val();
	var ngram = $("#Keyboard" + divIdCoreForKeyboard[keyboard] + "Container .gwt-TextBox").val();
	...
	var new_mel_str = ints.join(" ");
	$("#Keyboard" + divIdCoreForKeyboard[keyboard] + "Container .gwt-TextBox").val(new_mel_str);
...
function changeMelodyNote(noteIndex, amount,keyboard) {
...
function removeNote(noteIndex,keyboard) {
	var ints = getMelodyArray(0,keyboard);
	var divIdCoreForKeyboard = {"timeage":"TimeAge","timecountry":"TimeCountry","timepublications":"TimePublications","keypercent":"KeyPercent","countrieskey":"CountriesKey","timekey":"TimeKey","agekey":"AgeKey","ngramcomposer":"NgramComposer"};
	var ngramType =  $("#ngram" + divIdCoreForKeyboard[keyboard] + "Container select").val();
	var ngram = $("#Keyboard" + divIdCoreForKeyboard[keyboard] + "Container 
...
function scoreOnClick(e,keyboard) {
	if (getMelodyArray(1,keyboard) != null) {
		return;
	}
